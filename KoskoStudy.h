...
private:
	...
	void learnGesture(const QDomElement &gesture, const int &gestureNum);
	...
	void makeMassCentres(const QVector<int> &img
						 ...
						 , int &upperCM
						 , int &bottomCM);
	QDomElement saveLine(QDomDocument &kn, const int &epochNum, const int &lineNum);
	int mCountOfGestures, mCountOfANeurons, mCountOfBNeurons, mWidth, mHeight, mXCMPos, mYCMPos, mCountOfEpochs;
